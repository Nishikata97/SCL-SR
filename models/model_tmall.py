...
class Model_tmall(nn.Module):
    def __init__(self, args, adjacency):
        super(Model_tmall, self).__init__()
    ...
    def extract_topk_samples(self, original_sampled_items, sample_items_embedd, all_items_embedd):
        ...
        random_slices = torch.randint(low=10, high=self.num_coarse_sampling, size=(self.num_expanded_samples,)) 
    ...
    def loss_predict_attribute(self, inter_feat, all_cat_embedding, all_seller_embedding, all_brand_embedding, cats, sellers, brands, mask):
        ...
        scores_seller = torch.matmul(inter_feat1.view(-1, 300), all_seller_embedding.transpose(1, 0))
        scores_brand = torch.matmul(inter_feat1.view(-1, 300), all_brand_embedding.transpose(1, 0))

        '''scores_cat = scores_cat.view(self.batch_size, -1, all_cat_embedding.shape[0]) 
        scores_seller = scores_seller.view(self.batch_size, -1, all_seller_embedding.shape[0])
        scores_brand = scores_brand.view(self.batch_size, -1, all_brand_embedding.shape[0])'''

        label_cat = cats.view(-1)
        label_seller = sellers.view(-1)
        label_brand = brands.view(-1)

        mask = mask.float().view(-1) 
        mask = mask == 1
        label_cat = torch.masked_select(label_cat, mask)
        label_seller = torch.masked_select(label_seller, mask)
        ...
        scores_seller = torch.masked_select(scores_seller, mask.unsqueeze(-1).repeat(1, all_seller_embedding.shape[0]))
    ...
    def forward(self, inputs, targets, masks, attributes, viewed_items, bought_items):
        ...
        if self.dataset == 'Tmall':
            sess_emb_i = self.generate_sess_emb_npos(inter_feat, masks)
        elif self.dataset == 'CIKM19':
        ...
        attr_feat = self.Att_encoder(inputs, attributes, masks, self.cat_embedding, self.seller_embedding, self.brand_embedding) 
        if self.dataset == 'Tmall':
            sess_emb_a = self.generate_sess_emb_npos(attr_feat, masks)
        elif self.dataset == 'CIKM19':
        ...
        loss_cat, loss_seller, loss_brand = self.loss_predict_attribute(disen_attr_feat_1, self.cat_embedding.weight[1:],
                                                                        self.seller_embedding.weight[1:], self.brand_embedding.weight[1:], cats,
                                                                        sellers, brands, masks)
        loss_attribute = loss_cat + loss_seller + loss_brand
        ...
        loss_batch_sess_1 = info_nce_loss_overall(disen_attr_sess_embedd, attr_sess_embedd, disen_attr_sess_embedd,
                                                  self.tau)
        ...
        '''
        loss1 = relation_nce_loss(inter_feat, comple_items_embedd,
                                  expanded_substi_items_embedd, masks, self.tau)

        loss2 = relation_nce_loss(attr_feat, substi_items_embedd, expanded_comple_items_embedd,
                                  masks, self.tau)
        loss_contra = loss1 + loss2'''

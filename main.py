...
from models.model_tmall import *
import wandb
import os
import argparse
import pickle
...
def main():
    ...
    item2cat, item2seller, item2brand = load_tmall_attribute(pre_dataset_dir)
    args.num_cats = max(item2cat.values()) + 1
    args.num_sellers = max(item2seller.values()) + 1
    args.num_brands = max(item2brand.values()) + 1
    print(args)
    wandb.init(name=time.strftime("%m-%d-%Hh%Mm%Ss-") + "-" + args.model, project=args.dataset)
    wandb.config.update(args)

    attribute_list = [item2cat, item2seller, item2brand]
    relation_list = [co_viewed_items, co_bought_items]
    train_data = Data(train_sessions, all_train, args.num_items, attribute_list, relation_list)
    test_data = Data(test_sessions, all_train, args.num_items, attribute_list, relation_list)

    model = trans_to_cuda(Model_tmall(args, train_data.adjacency))
